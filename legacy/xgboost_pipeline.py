...
def load_and_reduce_hsi(directory, pca=None, fit=False, n_components=10):
    raw_data = load_masked_mean(directory)
    if fit:
        pca = PCA(n_components=n_components)
        reduced = pca.fit_transform(raw_data)
        return reduced, pca
    else:
        reduced = pca.transform(raw_data)
        return reduced
...
X_airborne, pca_air = load_and_reduce_hsi(HSI_AIRBORNE_DIR, fit=True, n_components=10)
X_hsi_satellite, pca_hsi = load_and_reduce_hsi(HSI_SATELLITE_TRAIN_DIR, fit=True, n_components=10)
X_msi_satellite = X_msi_satellite_raw  # Already 13 bands

# %% Combine and split data
X_combined = apply_modality_dropout(X_airborne, X_hsi_satellite, X_msi_satellite, dropout_prob=0.5)
...
for i, col in enumerate(columns):
    ...
    model = XGBRegressor(n_estimators=300, random_state=42, n_jobs=2)
...
print(f"Final score using XGB with PCA with dropout (mean over all): {np.mean(scores):.4f}")

# %% Load and predict on test set
print("Preparing test set...")
X_hsi_satellite_test = load_and_reduce_hsi(HSI_SATELLITE_TEST_DIR, pca=pca_hsi)
